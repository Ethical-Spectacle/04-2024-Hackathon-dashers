...
def generate_path(origin_point, target_point, perimeter):
    ...
    # origin_node = ox.nearest_nodes(roadgraph, origin_point)
    origin_node, distance = ox.nearest_nodes(roadgraph, origin_point[1], origin_point[0], return_dist=True)

    # Get the nearest node in the OSMNX graph for the target point
    # target_node = ox.get_nearest_node(roadgraph, target_point)
    target_node, distance = ox.nearest_nodes(roadgraph, target_point[1], target_point[0], return_dist=True)
...
def plot_map(origin_point, target_points, long, lat):
    ...
    fig.update_layout(
        mapbox_style="light", 
        mapbox_accesstoken="sk.eyJ1IjoiZ3JleWhvdW5kOTciLCJhIjoiY2x1cGtpZmgzMHJ2NTJrcGJ6dm9qZGV6biJ9.s05ArrYI84QObO9CBbUGnA",
